...
function process_hubmap_data()
{
  ...
  */
  ...
  //console.log("res", res);
  var simple_jsn = {"n_top":res["n_top"], "n_total":res["n"], "split_top":split_top};
  simple_jsn["edges"] = [];
...
function generate_coordinates(all_nodes, n_top, node_id_to_txt)
{
  ...
  for(var i=0;i<=max_node;i++)
  {
    ...
    if(i<n_top)
    {
      ...
      nodes.push({"label":i, "txt":node_id_to_txt[i], "x":layer1_x, "y":left_y});
      //left_y += 200;
      left_y += height/n_top;
    }
    else
    {
      ...
      nodes.push({"label":i, "txt":node_id_to_txt[i], "x":layer2_x, "y":right_y});
      //right_y += 200;
...
function split_stats(G_json, bottom_order, bottom_degree)
{
  ...
  */
  ...
  console.log(n_top + " & " + n_bot + " & " + vertex_splits + " & " + split_vertices + " & " + max_split);
}
function text_to_nodes(node, split_top)
{
  node.append("text")
    //.attr("dx", 12)
    .attr("dx", function(d){
                   if(d.x==layer1_x)return -100;
                  else return 12;
      })
    .attr("dy", ".35em")
    //.text(function(d) { return d.label });
    .text(function(d) { return d.txt.substr(0, 10) });

  node.on("mouseover", function(){
    var cur_data = d3.select(this).data()[0];
    var rect_width = Math.max(("Name:" + cur_data.txt).length*8, ("Type: " + "Gene/protein").length*8);
    d3.select(this)
    .append("rect")
    .attr('x', 10)
    .attr('y', 20)
    ...
    .attr("dx", 12)
    .attr("dy", 95)
    .text(d=>"Degree: " + d3.selectAll("path").filter(d => d.source==cur_data || d.target==cur_data).data().length)
    .style("fill", "yellow");
  });
  node.on("mouseout", function(){
    d3.select("rect").remove();
    d3.select("#full_label_id").remove();
    d3.select("#type_id").remove();
    d3.select("#id_id").remove();
    d3.select("#degree_id").remove();
  });
}
function arc(d,i) {
    ...
    return draw([[layer1_x, d.source.y], [layer1_x+(layer2_x-layer1_x)/3, d.source.y], [layer2_x-(layer2_x-layer1_x)/3, d.target.y], [layer2_x, d.target.y]])
}
function draw_splines(all_edges)
{
...
function color_nodes(node, is_split)
{
  node.append("circle")
   //.style("fill", "red")
...
function draw()
{
  ...
  for(var i=0;i<G_json["edges"].length;i++)
  {
    ...
    if(all_nodes.findIndex(x => x==edge[0])==-1)
    {
      all_nodes.push(edge[0]);
      left_nodes.add(edge[0]);
    }
  ...
  color_nodes(node, G_json.split_top);

  text_to_nodes(node, G_json.split_top);

  draw_splines(all_edges);
  ...
  console.log(`Call to generate split graph took ${endTime - startTime} milliseconds`)
...
function split()
{
  ...
  */
  ...
  color_nodes(node, split_top);

  text_to_nodes(node, split_top);
